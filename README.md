...
**Customer POST returns 422 instead of 201:** A1 defines **422** only when **`userId` (email) already exists**. Common cases:
- **Stale rows:** Run **`scripts/truncate_customers.sql`** (or **`TRUNCATE TABLE customers;`**) on the **Aurora writer** after manual API testing or between experiments. **`init_db.sql`** drops/recreates tables but wipes books too; use **truncate only `customers`** if you want to keep book data.
- **Web test then Mobile test on Gradescope:** Both suites hit the **same RDS**. If the autograder uses the **same** `userId` for the Web “add customer” test and the Mobile “add customer” test, the **first** returns **201** and the **second correctly returns 422** per A1. That is an **implementation-consistent** outcome; if both are in one run and Mobile always fails, the course tests may expect isolated data (contact TA) or unique emails per case.
- The service stores **`userId` in lowercase** (trimmed) so **`UNIQUE`** and duplicate checks stay consistent.
...
6. **Mobile BFF (A2):** `non-fiction` → `3` on **single-book** responses: **GET** **`/books/{ISBN}`** / **`/books/isbn/{ISBN}`**, **POST `/books`** (**201** body), and **PUT `/books/{ISBN}`** (**200** body). **Not** on **GET `/books`** (list). Strip address fields only on **GET `/customers/{id}`** and **GET `/customers?userId=`**, **not** on **GET `/customers`**. **Location:** BFFs rewrite relative `Location` to **`http(s)://<Host>...`**. If **`url.txt`** uses **`http://alb:80`** but the **`Host`** header has no port, add **`-e BFF_HTTP_LOCATION_PORT=80`** to **web-bff** and **mobile-bff**, *or* drop **`:80`** from **`url.txt`**.
